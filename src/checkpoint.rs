...
pub(crate) fn snapshot_trailer<S: Sink>(raw: &RawWriter<S>) -> Result<TrailerSnapshot, WriteError> {
    ...
    if !raw.frames.is_empty() && raw.root_offset.is_some() {
        return Err(WriteError::MultipleRootValues);
    }

    let prefix_len = raw.pos;
    let mut tail: Vec<u8> = Vec::new();
    let mut tail_pos = raw.pos;
    let mut tail_padding = raw.padding_written;
    let mut scratch = Vec::with_capacity(64);
    let mut frames = raw.frames.clone();
    let mut root: Option<u64> = raw.root_offset;
    ...
    while let Some(frame) = frames.pop() {
        ...
        let root_off = close_frame(frame, &raw.opts.policy, &mut ctx)?;
        if let Some(parent) = frames.last_mut() {
            register_into_frame(parent, root_off, &raw.opts, &mut ctx)?;
        } else {
            root = Some(root_off);
        }
    }
    ...
    if let PageAlignment::Aligned { page_size } = raw.opts.policy.align {
